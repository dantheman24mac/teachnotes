...
ARG DEMO_MODE=false
ARG TEACHNOTES_ENVIRONMENT=production
ARG TEACHNOTES_RELEASE_SHA
ENV NEXT_PUBLIC_SUPABASE_URL=$NEXT_PUBLIC_SUPABASE_URL \
    NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY=$NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY \
    NEXT_PUBLIC_APP_URL=$NEXT_PUBLIC_APP_URL \
    DEMO_MODE=$DEMO_MODE \
    TEACHNOTES_ENVIRONMENT=$TEACHNOTES_ENVIRONMENT \
...
COPY --from=builder --chown=nextjs:nodejs /app/scripts/seed-staging.mjs ./scripts/seed-staging.mjs
USER nextjs
EXPOSE 3000
