  ...
  return (
    <>
      <div className="page-heading">
        <div><p className="eyebrow">Your teaching day</p><h1>Today’s agenda</h1><p className="subtle">{formatInWorkspaceTime(new Date(), timezone, { weekday: "long", day: "numeric", month: "long" })}</p></div>
        ...
        <div className="agenda-list">
          ...
          {lessons.map((lesson) => (
            <article className="agenda-card" key={lesson.id}>
              <div className="time-block"><strong>{formatInWorkspaceTime(lesson.startsAt, timezone, { hour: "2-digit", minute: "2-digit" })}</strong><span>{lesson.durationMinutes} min</span></div>
