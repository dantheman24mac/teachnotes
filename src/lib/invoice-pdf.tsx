...
import { formatInWorkspaceTime } from "./timezone";
...
function InvoiceDocument({ invoice, settings }: { invoice: Invoice; settings: BusinessSettings }) {
  return <Document title={invoice.number ?? "TeachNotes invoice"} author={settings.tutorName}>
    <Page size="A4" style={styles.page}>
      ...
      <View style={styles.columns}><View style={styles.column}><Text style={styles.label}>From</Text><Text style={styles.name}>{settings.tutorName}</Text><Text style={styles.line}>{settings.tutorEmail}</Text><Text style={styles.line}>{settings.tutorPhone}</Text><Text style={styles.line}>{settings.tutorAddress}</Text></View><View style={styles.column}><Text style={styles.label}>Bill to</Text><Text style={styles.name}>{invoice.recipientName}</Text><Text style={styles.line}>Period {formatInWorkspaceTime(invoice.periodStart, settings.timezone, { day: "numeric", month: "numeric", year: "numeric" })} – {formatInWorkspaceTime(invoice.periodEnd, settings.timezone, { day: "numeric", month: "numeric", year: "numeric" })}</Text><Text style={styles.line}>Due {invoice.dueAt ? formatInWorkspaceTime(invoice.dueAt, settings.timezone, { day: "numeric", month: "numeric", year: "numeric" }) : "On receipt"}</Text></View></View>
